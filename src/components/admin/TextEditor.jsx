...
const TextEditor = ({ data = "<p></p>", onChange = () => {} }) => {
  ...
  const editor = useEditor({
    ...
    // Required in Next: the first render happens on the server, where
    // ProseMirror has no DOM to attach to.
    immediatelyRender: false,
    shouldRerenderOnTransaction: false,
    // Required under Next: the first render happens on the server, where
    // ProseMirror has no DOM to attach to.
    immediatelyRender: false,
    content: data || "<p></p>",
    onUpdate: ({ editor }) => {
  ...
  const editorState = useEditorState({
    editor,
    // `editor` is null until it has been created in the browser, so every

...
import { generateSlug } from "@/lib/nav";
...
const Services = () => {
  ...
  const dummyColumns = [
    ...
    },
    {
      title: "Post date",
      dataIndex: "postDate",
      render: (value) => <p>{formatApiDate(value)}</p>,
    },
    {
      title: "Description",
            ...
            items={[
              {
                key: 1,
                label: (
                  <Link href={`/${userId}/admin/services/${record?.id}/detail`}>Add table of content</Link>
                ),
              },
              { key: 2, label: <Link href={`/${userId}/admin/services/${record?.id}/faqs`}>FAQ&apos;s</Link> },
